...
function InsufficientSheet({ balance, required, onClose }: { balance: number; required: number; onClose: () => void; }) {
  return (
    <div className="glass-card rounded-2xl p-8 text-center border border-yellow-500/20">
      ...
      <p className="text-sm text-muted-foreground mb-4">Silakan isi saldo terlebih dahulu di tab <span className="font-bold text-yellow-300">Isi Saldo</span>.</p>
      ...
      <button onClick={onClose}
        ...
        style={{ background: "linear-gradient(135deg, hsl(210 90% 55%) 0%, hsl(230 80% 45%) 100%)", boxShadow: "0 4px 15px rgba(59,130,246,0.3)" }}>
        Mengerti
      </button>
    </div>
  );
