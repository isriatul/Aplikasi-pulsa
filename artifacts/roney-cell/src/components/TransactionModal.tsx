  ...
  return (
    ...
    >
      <div className="w-full max-w-md modal-content">
        {phase === "confirm" && (
          <ConfirmSheet product={product!} phone={phone} operator={operator} balance={balance} onConfirm={onConfirm} onClose={onClose} />
        )}
        {phase === "loading" && <LoadingSheet />}
        {phase === "success" && (
          <ResultSheet success phone={phone} product={product} operator={operator} refId={refId} onClose={onClose} />
        )}
        {phase === "failed" && (
          <ResultSheet success={false} phone={phone} product={product} operator={operator} refId={refId} onClose={onClose} message={errorMessage} />
        )}
        {phase === "insufficient" && (
...
function ConfirmSheet({ product, phone, operator, balance, onConfirm, onClose }: {
  product: Product; phone: string; operator: Operator | null; balance: number; onConfirm: () => void; onClose: () => void;
}) {
  ...
  return (
    <div className="glass-card rounded-2xl p-6 border border-blue-500/20">
      ...
      <div className="space-y-3 mb-5">
        ...
        <Row label="Operator" value={operator?.name ?? "Tidak dikesan"} valueStyle={{ color: operator?.color }} />
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          style={canAfford ? { background: "linear-gradient(135deg, hsl(210 90% 55%) 0%, hsl(230 80% 45%) 100%)", color: "white", boxShadow: "0 4px 15px rgba(59,130,246,0.3)" } : undefined}
        >
          Hantar Transaksi
        </button>
      </div>
...
function Row({ label, value, valueClass, valueStyle }: {
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={`text-sm font-semibold text-foreground ${valueClass ?? ""}`} style={valueStyle}>{value}</span>
    </div>
  );
}
...
function ResultSheet({ success, phone, product, operator, refId, onClose, message }: {
  ...
  function sendWhatsApp() {
    ...
    const msg = buildOrderMessage(phone, product.name, product.price, operator?.name, refId ?? "-");
    window.open(buildWhatsAppUrl(msg, cfg.whatsappNumber), "_blank");
  }

  return (
    <div className={`glass-card rounded-2xl p-8 text-center border ${success ? "border-emerald-500/20" : "border-destructive/20"}`}>
      <div
        className={`w-20 h-20 rounded-full mx-auto mb-5 flex items-center justify-center`}
        style={{
          background: success ? "rgba(16,185,129,0.15)" : "rgba(239,68,68,0.1)",
          boxShadow: success ? "0 0 30px rgba(16,185,129,0.3)" : "0 0 30px rgba(239,68,68,0.2)",
        }}
      >
        {success ? (
      ...
      {success && refId && (
        <p className="text-xs text-muted-foreground mb-1">Ref ID: <span className="font-mono text-foreground">{refId}</span></p>
      )}
      <p className="text-sm text-muted-foreground mb-6">
        {success ? "Pulsa telah berjaya dihantar kepada pelanggan." : message ?? "Terdapat masalah semasa memproses transaksi."}
      </p>
      ...
      <div className="space-y-3">
        {success && cfg.whatsappNumber && (
          ...
          >
            ...
            Hantar Bukti ke WhatsApp Owner
          </button>
        )}
        <button
          onClick={onClose}
...
function InsufficientSheet({ balance, required, onClose }: { balance: number; required: number; onClose: () => void; }) {
  return (
    <div className="glass-card rounded-2xl p-8 text-center border border-destructive/20">
      ...
      <p className="text-sm text-muted-foreground mb-4">Saldo anda tidak mencukupi untuk transaksi ini.</p>
      ...
      <button onClick={onClose} className="w-full py-3.5 rounded-xl text-sm font-bold gradient-blue text-white transition-all"
